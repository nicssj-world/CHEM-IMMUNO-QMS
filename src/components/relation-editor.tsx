...
import { label, relationTypeLabels } from '@/lib/labels';
...
export function RelationEditor({ sourceId, options, relation }: { sourceId: string; options: ProductOption[]; relation?: Relation }) {
  ...
  return <form action={saveRelation} className="grid gap-3 rounded-xl border border-line p-4">
    <input type="hidden" name="source_product_id" value={sourceId}/>{relation && <input type="hidden" name="id" value={relation.id}/>}
    <div className="grid gap-3 sm:grid-cols-2"><label className="field">ความสัมพันธ์จากสินค้านี้<select name="relation_type" className="input" value={kind} onChange={e => setKind(e.target.value)}>{relationTypes.map(t => <option key={t} value={t}>{label(relationTypeLabels,t)}</option>)}</select></label><label className="field">ค้นหาเป้าหมาย<input className="input" value={query} onChange={e => setQuery(e.target.value)} placeholder="Product Code / ชื่อ"/></label></div>
    ...
    <div><SubmitButton className="button" label={relation ? 'บันทึกความสัมพันธ์' : 'เพิ่มความสัมพันธ์'}/></div>
  </form>;
}
