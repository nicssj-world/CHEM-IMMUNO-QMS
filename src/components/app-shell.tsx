...
export function AppShell({ access, children }: { access: AccessContext; children: React.ReactNode }) {
  ...
  return <div className="app-grid shell">
    ...
    <div className="main-area"><header className="topbar"><Link href="/" className="font-extrabold text-[1.05rem] no-underline text-[var(--ink)] md:hidden">CHEM-IMMUNO <span className="text-[var(--teal)]">CBH</span></Link><p className="muted text-sm hidden md:block">ระบบคลัง Clinical Chemistry และ Immunology</p><div className="flex items-center gap-3"><div className="text-right"><p className="text-sm font-bold">{access.ephisId}</p><p className="text-[.7rem] uppercase tracking-wide muted">{access.warehouses.map(w => `${w.code}: ${w.role}`).join(' · ')}</p></div><form action={signOut}><button type="submit" className="button secondary text-xs px-3" aria-label="ออกจากระบบ">ออก</button></form></div></header><div className="pt-7">{children}</div></div>
    <nav className="bottom-nav" aria-label="เมนูมือถือ"><Link href="/"><House size={21}/>หน้าหลัก</Link><Link href="/stock"><Boxes size={21}/>คลัง</Link><Link href="/receive"><ScanLine size={21}/>รับเข้า</Link>{canWork ? <Link href="/issue"><ArrowUpFromLine size={21}/>เบิก</Link> : <Link href="/movements"><History size={21}/>ประวัติ</Link>}<Link href="/products"><MoreHorizontal size={21}/>สินค้า</Link></nav>
  </div>;
}
