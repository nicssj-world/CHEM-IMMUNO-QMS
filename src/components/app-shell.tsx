...
export function AppShell({ access, children }: { access: AccessContext; children: React.ReactNode }) {
  ...
  return <div className="app-grid shell">
    ...
    <div className="main-area"><header className="topbar"><Link href="/" className="font-extrabold text-[1.05rem] no-underline text-[var(--ink)] md:hidden">CHEM-IMMUNO <span className="text-[var(--teal)]">CBH</span></Link><p className="muted text-sm hidden md:block">ระบบคลัง Clinical Chemistry และ Immunology</p><div className="flex items-center gap-3"><Link href="/account" className="text-right no-underline text-[var(--ink)] min-h-11 grid content-center" aria-label="บัญชีของฉัน / เปลี่ยนรหัสผ่าน"><p className="text-sm font-bold">{access.displayName}</p><p className="text-[.7rem] uppercase tracking-wide muted">{access.ephisId} · {access.warehouses.map(w => `${w.code}: ${w.role}`).join(' · ')}</p></Link><form action={signOut}><button type="submit" className="button secondary text-xs px-3 min-h-11" aria-label="ออกจากระบบ">ออก</button></form></div></header><div className="pt-7">{children}</div></div>
