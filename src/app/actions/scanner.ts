...
export type ScanResolution = { parsed: ParsedBarcode; productId?: string; productCode?: string; invoiceLineId?: string; message?: string; scanId?: string };
...
export async function resolveScan(raw: string, symbology: string, warehouseId: number, invoiceId?: string): Promise<ScanResolution> {
  ...
  if (ids.size === 1) {
    ...
    if (matchedWarehouseId !== warehouseId) result.message = 'Barcode belongs to another warehouse; change warehouse context before receiving';
    else {
      ...
      if (invoiceId) {
        ...
        else result.message = 'Barcode Product is not on this invoice';
      }
    }
  } else result.message = ids.size ? 'Barcode maps to multiple approved Products; supervisor review required' : 'ไม่พบสินค้าที่ตรงกับ Barcode';
...
export async function registerInvoiceAttachment(invoiceId: string, warehouseId: number, file: { type: string; size: number }) {
  ...
  if (error || !data?.[0]) throw new Error(error?.message ?? 'Unable to register invoice photo');
  ...
  if (uploadError || !upload) throw new Error(uploadError?.message ?? 'Unable to authorize photo upload');
...
export async function removeInvoiceAttachment(attachmentId: string) {
  ...
  if (lookupError || !attachment) throw new Error(lookupError?.message ?? 'Attachment not found');
...
export async function resolveProductScan(raw: string, symbology: string): Promise<ProductScan> {
  ...
  } else result.message = ids.size ? 'Barcode ตรงกับหลายสินค้า · ต้องให้ Supervisor ตรวจสอบ' : 'ไม่พบสินค้าที่ตรงกับ Barcode · เลือกสินค้าเอง';
