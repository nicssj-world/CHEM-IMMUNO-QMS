...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="min-h-dvh flex items-center justify-center px-4 py-10 bg-[radial-gradient(circle_at_top_right,#bcefe7,transparent_38%),linear-gradient(160deg,#eaf4f7,#f8fbfa)]">
    <section className="surface w-full max-w-[440px] p-7 sm:p-9">
      ...
      <form action={signIn} className="grid gap-5" autoComplete="off">
        <label className="field">Ephis ID<input className="input" name="ephisId" autoComplete="one-time-code" required /></label>
        <label className="field">รหัสผ่าน<input className="input" name="password" type="password" autoComplete="new-password" required /></label>
