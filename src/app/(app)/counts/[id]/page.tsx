...
export default async function CountDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  return <main className="grid gap-6 max-w-[1000px]"><div><Link href={`/counts?warehouse=${warehouse.code}`} className="muted text-sm">← รอบตรวจนับ</Link><p className="eyebrow mt-5 mb-2">Physical count · {warehouse.name}</p><h1 className="page-title">รอบตรวจนับ</h1><p className="muted mt-2 text-sm">{new Date(count.created_at).toLocaleString('th-TH')} · {count.note || 'ไม่มีหมายเหตุ'}</p><p className="mt-3"><span className="badge">{count.status}</span></p></div>{query.error && <p className="error" role="alert">{query.error}</p>}{query.saved && <p className="notice" role="status">บันทึกแล้ว</p>}{lineError && <p className="error">{lineError.message}</p>}
    <section className="grid gap-3">{lines.map(line => { const product = line.ci_stock_lots ? products.get(line.ci_stock_lots.product_id) : null; const variance = line.physical_quantity == null ? null : Number(line.physical_quantity) - Number(line.snapshot_quantity); return <article className="surface p-4 sm:p-5" key={line.id}><div className="flex justify-between gap-3 flex-wrap"><div><strong>{product?.product_code ?? '—'} · {product?.display_name ?? '—'}</strong><p className="muted text-xs mt-1">LOT {line.ci_stock_lots?.lot_number ?? '—'} · {line.ci_locations?.code ?? '—'}</p></div><div className="text-right text-sm"><p>Snapshot <strong>{Number(line.snapshot_quantity)}</strong></p><p>ส่วนต่าง <strong>{variance == null ? '—' : variance > 0 ? `+${variance}` : variance}</strong></p></div></div>{count.status === 'draft' && canMutate(warehouse.role) ? <form action={setCountLine} className="flex flex-wrap gap-3 items-end mt-4"><input type="hidden" name="count_id" value={id}/><input type="hidden" name="line_id" value={line.id}/><label className="field flex-1 min-w-[150px]">ยอดตรวจนับจริง<input className="input" name="physical_quantity" type="number" min="0" step="0.001" defaultValue={line.physical_quantity ?? ''} required/></label><button className="button secondary" type="submit">บันทึกยอด</button></form> : <p className="mt-3 text-sm">ยอดจริง <strong>{line.physical_quantity ?? '—'}</strong></p>}</article>; })}{lines.length === 0 && <p className="notice">รอบนี้ไม่มี LOT ในขอบเขต</p>}</section>
    {count.status === 'draft' && canSupervise(warehouse.role) && <ConfirmForm action={approveCount} message="ยืนยันอนุมัติผลตรวจนับและสร้าง adjustment ตามส่วนต่าง? หาก ledger เปลี่ยนหลัง snapshot ระบบจะหยุด" className="surface p-5 sm:p-7 grid gap-4"><input type="hidden" name="count_id" value={id}/><h2 className="font-bold text-lg">อนุมัติผลตรวจนับ</h2><p className="muted text-sm">ต้องกรอกยอดจริงทุกรายการ ฐานข้อมูลตรวจ snapshot ใหม่ใน transaction</p><label className="field">เหตุผลอนุมัติ<input className="input" name="reason" required/></label><div><button className="button" type="submit">อนุมัติและปรับยอด</button></div></ConfirmForm>}
  </main>;
}
