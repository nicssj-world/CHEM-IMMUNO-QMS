...
import { label, productTypeLabels, stockStatusLabels } from '@/lib/labels';
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ warehouse?: string; q?: string; type?: string; platform?: string; stock?: string; expiry?: string; page?: string; error?: string }> }) {
  ...
  return <main className="grid gap-6"><div className="flex flex-wrap justify-between items-end gap-4"><div><p className="eyebrow mb-2">Product master</p><h1 className="page-title">สินค้า</h1><p className="muted mt-2 text-sm">ค้นด้วย Product Code, REF, Barcode หรือชื่อสินค้า · {warehouse.name}</p></div>{canSupervise(warehouse.role) && <Link href={`/products/new?warehouse=${warehouse.code}`} className="button"><Plus size={18}/>เพิ่มสินค้า</Link>}</div>
    ...
    {params.error && <p className="error" role="alert">{params.error}</p>}{(error||platformResult.error) && <p className="error" role="alert">โหลดสินค้าไม่สำเร็จ: {logUserMessage('products', error||platformResult.error)}</p>}
    {!error && <section className="surface overflow-hidden"><div className="px-5 py-4 flex justify-between"><h2 className="font-bold">รายการสินค้า</h2><span className="muted text-sm">หน้า {page} · {rows.length} รายการ</span></div><div className="desktop-table table-wrap"><table className="data-table"><thead><tr><th>Product Code</th><th>ชื่อสินค้า</th><th>ประเภท</th><th>ขนาดบรรจุ</th><th>Usable</th><th>Stock</th></tr></thead><tbody>{rows.map(p=><tr key={p.id}><td><Link href={`/products/${p.id}`} className="font-bold text-brand">{p.product_code}</Link></td><td>{p.display_name}</td><td>{label(productTypeLabels,p.product_type)}</td><td>{p.packing_size_raw||'—'}</td><td>{Number(p.usable_stock).toLocaleString()}</td><td>{label(stockStatusLabels,p.stock_status)}</td></tr>)}</tbody></table></div><div className="mobile-card-list px-4 pb-4">{rows.map(p=><Link key={p.id} href={`/products/${p.id}`} className="border border-line rounded-xl p-4 no-underline text-[var(--ink)]"><div className="flex justify-between gap-2"><strong className="text-brand">{p.product_code}</strong><span className="badge">{label(productTypeLabels,p.product_type)}</span></div><p className="font-bold mt-2">{p.display_name}</p><p className="muted text-sm">{p.packing_size_raw||'—'} · ใช้ได้ {Number(p.usable_stock).toLocaleString()} · {label(stockStatusLabels,p.stock_status)}</p></Link>)}</div>{rows.length===0 && <p className="muted px-5 pb-5">ไม่พบสินค้า</p>}</section>}
