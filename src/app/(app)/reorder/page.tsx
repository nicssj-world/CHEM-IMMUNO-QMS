...
export default async function ReorderPage({ searchParams }: { searchParams: Promise<{warehouse?:string;q?:string;page?:string;error?:string;saved?:string}> }) {
  ...
  let query = client.from('ci_products').select('id,product_code,display_name').eq('warehouse_id',warehouse.id).eq('active',true).order('product_code').range(from,from+PAGE_SIZE);
