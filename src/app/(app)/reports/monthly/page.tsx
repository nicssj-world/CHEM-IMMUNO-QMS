...
const reportColumnLabels = { opening: 'ยอดยกมา', received: 'รับเข้า', issued: 'เบิกใช้', adjustments: 'ปรับยอด', expired_disposal: 'กำจัดหมดอายุ', reversals: 'ยกเลิกรายการ', closing: 'ยอดคงเหลือปลายเดือน' } as const;
...
export default async function MonthlyReportPage({ searchParams }: { searchParams: Promise<{ warehouse?: string; month?: string }> }) {
  ...
  const unitOf = new Map((unitRows.data??[]).map(u=>[u.id as string,u.base_stock_unit as string]));
