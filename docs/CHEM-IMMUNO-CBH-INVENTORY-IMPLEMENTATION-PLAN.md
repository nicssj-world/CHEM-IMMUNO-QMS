...
Vendor Master, receipt assessment, issues, performance และ annual evaluation ใช้หลักฐานรับเข้าและช่วงปีงบประมาณ **1 ต.ค.–30 ก.ย.** **การตัดสินของเจ้าของ (2026-09-25): ระบบประเมินผู้ขายใช้ตาม LABCBH-Stock ทั้งชุดเป็น QP ของกลุ่มงาน** — ผลตรวจรับ 5 เกณฑ์ + เหตุผล, ปัญหาผู้ขาย 8 ประเภท, นโยบาย 8 เกณฑ์ถ่วงน้ำหนัก (ใช้ร่วมทั้งกลุ่มงาน อนุมัติโดยผู้ดูแลระบบทั้งสองคลัง), รายงานประจำปีแบบตรึงข้อมูล (revision, เลข `VE-{พ.ศ.}-{0001}`, ลายเซ็น 3 ช่อง) และ PDF ตามแบบ LABCBH ประเมินแยกตามคลัง (CHE/IMM) และปีงบประมาณในส่วนนี้เป็น พ.ศ. คะแนนอย่างเป็นทางการมีได้เฉพาะรายงานที่ final บนนโยบายที่อนุมัติแล้ว (ก่อนนั้นแสดงเฉพาะข้อมูลสดที่ไม่ใช่คะแนน) ใบรับก่อนวันเริ่มใช้ระบบตรวจรับถือเป็น "ไม่ต้องประเมิน" และเพิ่มการ "ปิด Invoice แบบรับไม่ครบ" เพื่อให้เกณฑ์การส่งมอบมีความหมายเหมือน LABCBH Attention center รวบรวม stock หมด, ต่ำกว่า ROP, expiry และ discrepancy/vendor issue โดยไม่รบกวนด้วย modal ซ้ำ
...
Schema ใช้ versioned migrations ที่ผ่าน disposable local PostgreSQL ก่อน Production; ไม่มี Staging หรือ Preview Supabase/Vercel environment และทุก Production rollout ต้องได้รับอนุมัติแยกอย่างชัดเจนก่อน apply migration หรือ import ข้อมูล Initial data import เป็น batch แยกจาก schema migration ไม่แตะ Stock-BM และไม่ใช้ Excel เก่า การย้อนแอปใช้ Vercel deployment ก่อนหน้า; การแก้ schema หลังเปิดใช้เป็น forward corrective migration; การแก้ stock หลัง confirm ใช้ reversal/adjustment พร้อม audit และ backup/reconciliation ก่อน cutover
...
**Production verification:** หลังได้รับอนุมัติแยกสำหรับ rollout ให้ยืนยัน Production project identity, สำรองข้อมูล และตรวจ Ephis login → เลือกคลัง → invoice เดียวที่มีสองคลัง → ถ่ายรูป → สแกน/resolve → LOT/expiry → confirm receipt → ยอดเพิ่ม → FEFO issue → ยอดลด → reorder/expiry attention → count/adjustment/reversal → audit → รายงานรายเดือน reconcile ทุกขั้นตอนต้องทำได้โดยไม่แก้ DB ด้วยมือ ทดสอบ migrations, import และ application flows ใน local disposable environments ก่อน Production authorization; ติดตาม error/latency/failed RPC และเตรียม rollback deployment

**การตัดสินที่ยังต้องได้ก่อน gate ที่เกี่ยวข้อง:** วิธี provision Ephis ID กับ Supabase Auth; นโยบาย vendor scoring ปิดแล้วด้วยการตัดสินของเจ้าของ 2026-09-25 (ใช้ตาม LABCBH-Stock; ต้องกรอกน้ำหนักและอนุมัติข้อเสนอ `VE-POLICY-V1` ตาม QP ก่อนมีคะแนนทางการ) ส่วนการ provision Ephis ID ยังเป็น hold; ส่วน REL-01 ถึง REL-12 และ PM-01 ถึง PM-08 มี owner decision บันทึกแล้วและไม่ใช่ hold ของ Product Master/import อีกต่อไป
