...
test('PWA manifest is standalone with installable icon sizes',async()=>{
  ...
  const approvedMaster=await readFile('assets/branding/chem-immuno-cbh-approved-master.png');
  assert.equal(createHash('sha256').update(approvedMaster).digest('hex').toUpperCase(),'EDB31F8C9B25698C734A9FFE97169D074EADC90D3F5C1BCEBB76915DCAE57525');
